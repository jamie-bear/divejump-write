  ...
  return (
    <svg width="100" height="100" viewBox="0 0 100 100" className="mx-auto">
      ...
      <circle cx="50" cy="50" r={r} fill="none"
        stroke={done ? '#2E5747' : '#3A6E82'} strokeWidth="8"
        strokeDasharray={circ} strokeDashoffset={offset}
        strokeLinecap="round" transform="rotate(-90 50 50)"
        style={{ transition: 'stroke-dashoffset 0.5s ease' }} />
      <text x="50" y="46" textAnchor="middle" fontSize="18" fontWeight="700" fill={done ? '#2E5747' : '#374151'}>
        {pct}%
      </text>
    ...
    <div className="flex flex-col items-center">
      <svg width="130" height="130" viewBox="0 0 130 130">
        ...
        <circle cx="65" cy="65" r={r} fill="none"
          stroke={done ? '#2E5747' : '#3A6E82'} strokeWidth="10"
          strokeDasharray={circ} strokeDashoffset={offset}
          strokeLinecap="round" transform="rotate(-90 65 65)"
          style={{ transition: 'stroke-dashoffset 0.6s ease' }} />
        {/* Center text */}
        <text x="65" y="58" textAnchor="middle" fontSize="13" fontWeight="700" fill={done ? '#2E5747' : '#1c1917'}>
          {written.toLocaleString()}
        </text>
        <text x="65" y="72" textAnchor="middle" fontSize="9" fill="#a8a29e">
          of {goal.toLocaleString()}
        </text>
        <text x="65" y="84" textAnchor="middle" fontSize="9" fill={done ? '#2E5747' : '#a8a29e'}>
          {done ? '✓ Done!' : 'words'}
        </text>
      </svg>
    ...
    <div className="flex items-center gap-2">
      ...
      <div className="flex-1 h-2 bg-stone-200 rounded-full overflow-hidden">
        <div className={`h-full rounded-full`}
          style={{ width: `${pct}%`, transition: 'width 0.3s', background: done ? '#2E5747' : '#3A6E82' }} />
      </div>
      <span className="text-xs text-stone-500 w-16 text-right flex-shrink-0">
        {goal.wordsWritten.toLocaleString()}
      </span>
      {done && <CheckCircle2 size={13} className="text-dj-teal flex-shrink-0" />}
    </div>
  );
}
  ...
  return (
    <div className="w-72 flex-shrink-0 bg-white border-l border-stone-200 flex flex-col h-screen overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-stone-200">
        <div className="flex items-center gap-2">
          <Target size={15} className="text-dj-prussian" />
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              ...
              className="text-xs text-dj-prussian hover:underline"
            >
          ...
          {editWordGoal && (
            <div className="flex gap-2 mt-3">
              <input type="number" min="1000" step="1000" value={wordGoalInput}
                ...
                className="flex-1 border border-stone-300 rounded px-2 py-1 text-sm outline-none focus:ring-1 focus:ring-dj-prussian" />
              <button onClick={handleSaveWordGoal}
                className="px-3 py-1 bg-dj-prussian text-white text-xs rounded hover:bg-dj-teal transition-colors">
                Save
              </button>
            </div>
          )}
                ...
                <button key={n} onClick={() => setWordCountGoal(n)}
                  className={`text-xs px-2 py-0.5 rounded-full border transition-colors ${
                    book.wordCountGoal === n
                      ? 'bg-dj-prussian text-white border-dj-prussian'
                      : 'border-stone-300 text-stone-500 hover:border-dj-prussian hover:text-dj-prussian'
                  }`}>
                  {(n / 1000).toFixed(0)}k
                </button>
              ))}
                ...
                {[500, 1000, 1500, 2000].map((n) => (
                  <button key={n} onClick={() => setDailyGoal(n)}
                    className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${
                      book.dailyGoal === n
                        ? 'bg-dj-prussian text-white border-dj-prussian'
                        : 'border-stone-300 text-stone-600 hover:border-dj-prussian hover:text-dj-prussian'
                    }`}>
                    {n.toLocaleString()}
                  </button>
                ))}
